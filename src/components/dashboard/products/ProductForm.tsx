...
export const ProductForm = ({ product, onSuccess, onCancel }: ProductFormProps) => {
    ...
    const handleAnalyzeImage = async () => {
        if (!previewUrl && !imageUrl) {
            toast.error('Sube una imagen primero para analizarla.');
            return;
        }
    ...
    const handleGenerateDescription = async () => {
        ...
        try {
            // Obtener talles disponibles para el prompt
            const availableSizes = Object.entries(stock)
                .filter(([_, qty]) => qty > 0)
                .map(([size]) => size);

            const stockSummary = availableSizes.length > 0
                ? `Talles actualmente disponibles: ${availableSizes.join(', ')}.`
                : 'Actualmente sin stock físico (próximo ingreso).';
      ...
      - Solo devolvé el texto limpio, sin títulos.`;
            ...
            if (generatedText) {
                ...
                toast.error('La IA no devolvió contenido válido.');
            }
        } catch (err: any) {
            console.error('Gemini AI Error:', err);
            toast.error(`Error al conectar con Gemini: ${err.message || 'Error de red'}`);
        } finally {
                ...
                <div className="space-y-5">
                    <div>
                        <div className="flex justify-between items-center mb-1.5">
                            ...
                            >
                                {analyzingName ? <Loader2 className="animate-spin" size={12} /> : <Sparkles size={12} />}
                                {analyzingName ? 'Analizando...' : 'Identificar con IA'}
                            </button>
                        </div>
                        <input
                            required
