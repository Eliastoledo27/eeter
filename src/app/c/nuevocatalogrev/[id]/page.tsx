...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
    ...
    if (!product) {
        return {
            title: 'Producto no encontrado | Ã‰TER Store'
        }
    }

    return {
        title: product.name,
        description: `Adquiere ${product.name} en Ã‰TER Store. CatÃ¡logo premium y logÃ­stica de primer nivel.`,
        openGraph: {
            title: `${product.name} | Ã‰TER Store`,
            description: `Descubre ${product.name}. Calzado luxury en stock. EnvÃ­o express.`,
...
export default async function ProductDetailPage({
    ...
    return (
        <div className="min-h-screen bg-[#050505] text-white overflow-x-hidden">
            ...
            <script
                ...
                dangerouslySetInnerHTML={{
                    __html: JSON.stringify({
                        ...
                        "description": `Adquiere ${product.name} en Ã‰TER Store. Calzado premium importado con calidad G5/OG.`,
                        "brand": {
                            "@type": "Brand",
                            "name": "Ã‰TER"
                        },
                        "offers": {
                            ...
                            "url": `https://eter-store.vercel.app/resellers/${product.id}`,
            ...
            {/* Trust Bar â€” Bold & Colorful */}
            <section className="relative z-10 border-t border-[#252525] bg-[#050505] px-6 py-16">
                <div className="container mx-auto max-w-7xl">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {[
                            { icon: Shield, label: 'Pago Seguro', desc: 'EncriptaciÃ³n de extremo a extremo en todas las transacciones.', color: '#00E5FF' },
                            { icon: Package, label: 'EnvÃ­o Express', desc: 'Despacho prioritario en 24hs hÃ¡biles a todo el paÃ­s.', color: '#00B8D9' },
                            { icon: Truck, label: '100% Original', desc: 'GarantÃ­a de autenticidad en todos nuestros productos.', color: '#00E0FF' }
                        ].map((item, idx) => (
