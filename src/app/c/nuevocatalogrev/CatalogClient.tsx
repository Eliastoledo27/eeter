...
export default function CatalogPage({ initialProducts }: CatalogClientProps) {
    ...
    const { loading: catalogLoading, categories: dbCategories, products: dbProducts } = useCatalog({
        initialProducts: mappedInitial,
        fetchOnMount: mappedInitial.length === 0,
        realtime: true
    });
    ...
    return (
        ...
        >
                    ...
                    <div className="mx-auto max-w-[1600px] relative z-10">
                        <div className="grid grid-cols-1 gap-12 lg:grid-cols-[minmax(0,1fr)_340px]">
                            <div className="min-w-0 order-2 lg:order-1">
                                ...
                                >
                                    <div className="mb-5 flex flex-col justify-between gap-5 md:flex-row md:items-center">
                                        <div>
                                            ...
                                            <p className="mb-2 text-[10px] font-black uppercase tracking-[0.28em] text-rose-400">
                                                Ofertas Flash
                                            </p>
                                            <h1 className="text-4xl font-black uppercase tracking-tight text-white md:text-5xl">
                                                CatÃ¡logo <span className="text-tri-gradient">liquidaciÃ³n</span>
                                            </h1>
                                        </div>
                                ...
                                <LiquidationCardsSection products={products} title="Productos en LiquidaciÃ³n" />
                            ...
                            <aside className="hidden lg:block order-2 lg:order-2 lg:pl-10 lg:border-l lg:border-white/5 relative h-full space-y-12">
            ...
            {/* BotÃ³n flotante mÃ³vil para filtros */}
            <div className="fixed bottom-6 left-1/2 z-[40] -translate-x-1/2 lg:hidden">
                <button
                    onClick={() => setIsFilterDrawerOpen(true)}
                    className="flex items-center gap-2 rounded-full border border-white/10 bg-black/60 px-6 py-3 text-xs font-black uppercase tracking-[0.2em] text-[#00E5FF] shadow-[0_0_25px_rgba(0,229,255,0.3)] backdrop-blur-20 transition-all hover:scale-105 active:scale-95"
                >
            ...
            <MobileFilterDrawer open={isFilterDrawerOpen} onOpenChange={setIsFilterDrawerOpen}>
                <FilterSidebar
                    categories={categories}
                    activeCategory={activeCategory}
                    onCategoryChange={setActiveCategory}
