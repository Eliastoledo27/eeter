...
import { Eye, EyeOff, Mail, Lock, LogIn, Loader2, AlertCircle, CheckCircle2 } from 'lucide-react'
...
export default function LoginForm() {
    ...
    const handleSubmit = useCallback(async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (data.user) {
                ...
                router.push(redirectTo || getDefaultRedirect(role))
                router.refresh()
            }
        } catch {
    ...
    }, [email, password, checkSession, router, searchParams])
            ...
            <div className="backdrop-blur-2xl bg-white/[0.03] border border-white/10 rounded-2xl p-6 sm:p-8 shadow-2xl">
                ...
                <form onSubmit={handleSubmit} className="space-y-5" noValidate>
                    ...
                    <div className="space-y-1.5">
                        <label htmlFor="login-email" className="text-xs font-semibold uppercase tracking-wider text-gray-400 ml-1">
                            Email
                        </label>
                  ...
                  ${errors.email ? 'border-red-500/50' : 'border-white/10 hover:border-white/20'}`}
                        ...
                        {errors.email && (
                            <p id="login-email-error" className="text-xs text-red-400 ml-1" role="alert">{errors.email}</p>
                        )}
                    </div>

                    {/* Password */}
                    <div className="space-y-1.5">
                        <label htmlFor="login-password" className="text-xs font-semibold uppercase tracking-wider text-gray-400 ml-1">
                            Contraseña
                        </label>
                        <div className="relative">
                  ...
                  ${errors.password ? 'border-red-500/50' : 'border-white/10 hover:border-white/20'}`}
                        ...
                        {errors.password && (
                            <p id="login-password-error" className="text-xs text-red-400 ml-1" role="alert">{errors.password}</p>
                        )}
